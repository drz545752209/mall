...
public class JedisUtils implements ApplicationContextAware {
     ...
     */
    public void BLPOP(String key) throws InterruptedException {
        jedis=getJedis();
     ...
     * 超时参数 timeout 接受一个以秒为单位的数字作为值。超时参数设为 0 表示阻塞时间可以无限期延长(block indefinitely) 。
     */
    public void RPOPLPUSH(String key) {
        jedis=getJedis();
     ...
     */
    public void LINDEX(String key,Integer index) {
        jedis=getJedis();
        System.out.println(jedis.lindex(key, index));
    }
     ...
     * 如果 key 不是列表类型，返回一个错误。
     */
    public void LINSERT(String key,String targeValue,String value) {
        jedis=getJedis();
        jedis.linsert(key, BinaryClient.LIST_POSITION.AFTER, targeValue, value);
    }
